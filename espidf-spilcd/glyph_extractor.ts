...
var stringFromCharCode =(charcode:number)=>{
	...
	function utf8decode(byteString:string) {
...
const buffer = fs.readFileSync('fa-regular-400.ttf');
...
var glyphs =font.find({filter:(gl)=>{return gl.unicode &&gl.unicode.length<=1}});
console.log(glyphs.length);
glyphs.forEach((e)=>{
    console.log("constexpr const char* GLYPH_"+e.name.replaceAll("-", "_").toLocaleUpperCase()+" = \""+encodeCodePoint(e.unicode[0])+"\";");
});
