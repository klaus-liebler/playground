...
void i2cSetupNAU8822Play(uint8_t initialVolume_0_255=255)//this is the hardware default volume
{
    ...
    else if (nauInit == eNauInit::ORIGINAL_DF_ROBOT)
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(100));
        i2cWriteNAU8822(1, 0x1FF);//AUX1+2 Mixer,PLL, MIc Amplifier enable (mache ich nicht) -->Warum aktiviere ich die AUX-Mixer nicht? Den "nternal Tie-off Buffer In Non-boost 1.0" möchte ich aber tatsächlich nicht haben!
        i2cWriteNAU8822(2, 0x1BF);//hier werden noch die ADC akiviert und auch die Eingangsverstärker -->ok
        i2cWriteNAU8822(3, 0x1FF);//Hier werden noch AUXOUT1+2 und ein Reserved-Bit aktiviert
        i2cWriteNAU8822(4, 0x010);//Exakt wie oben
        i2cWriteNAU8822(5, 0x000);//Das ist der Standard-Wert!
        i2cWriteNAU8822(6, 0x00C);//Exakt wie oben, irgendwelche Clocks
        i2cWriteNAU8822(7, 0x000);//Das ist der Standardwert
        i2cWriteNAU8822(13, 0x09f);//Jack==1: AUX1, Headphones; Jack==0: Alles aktiv (aber: JAck Detect Feature nicht aktiv!!!)
        i2cWriteNAU8822(10, 0x008);//DAC bekommt 128x oversampling, ich könnte das natürlich auch machen
        i2cWriteNAU8822(14, 0x108); //ADC bekommt einen speziellen Filter und 128x Oversampling
        i2cWriteNAU8822(15, 0x0FF); //ADC Left Volume -->Standardwert
        i2cWriteNAU8822(16, 0x1FF); //ADC Right Volume --> Standardwert und Update-Bit
        //BTL wird in Register 43 gesetzt, hier nicht... -->wesentlicher Unterschied
        i2cWriteNAU8822(45, 0x0bf); //PGA left input ganz laut!
        i2cWriteNAU8822(46, 0x1bf); //auch für rechts +  Update
        i2cWriteNAU8822(47, 0x175); //ADC +20db Booster, ganz laut
        i2cWriteNAU8822(49, 0x042); //Spannend 0b0'0100'0010 . Ich aktiviere durchgängig den 5V-Boost
        i2cWriteNAU8822(50, 0x1DD); //Ich setze auf default, er hier macht LAUXIN ganz laut LADC to LMAIN auch ganz laut
        i2cWriteNAU8822(51, 0x001); //Für Raux auf Standard, ich deaktiviere "RDAC Output To RMAIN MIXER Path Enable Control"
    }
