...
namespace webmanager
{
    ...
    private:
        ...
        esp_err_t sendResponseJournal(httpd_req_t *req, httpd_ws_frame_t *ws_pkt)
        {
            ...
            for (int i = 0; i < BUFFER.size(); i++)
            {
                if (BUFFER[i].messageCode == 0)
                    continue;
                auto itemOffset=CreateJournalItemDirect(b, BUFFER[i].lastMessageTimestamp, BUFFER[i].messageCode, messagecodes::N[BUFFER[i].messageCode], BUFFER[i].lastMessageData, BUFFER[i].messageCount);
        ...
        static esp_err_t handle_ota_post_static(httpd_req_t *req)
        {
            ...
            return myself->handle_ota_post(req);
        }
