...
void bt_app_a2d_data_cb(const uint8_t *data, uint32_t len)
{
    ...
    if (++s_pkt_cnt % 100 == 0) {
        ESP_LOGI(BT_AV_TAG, "Audio packet count %lu", s_pkt_cnt);
    }
}
...
void bt_av_notify_evt_handler(uint8_t event_id, esp_avrc_rn_param_t *event_parameter)
{
    ...
    case ESP_AVRC_RN_PLAY_STATUS_CHANGE:
        ESP_LOGI(BT_AV_TAG, "Playback status changed: 0x%x", (int)event_parameter->playback);
        bt_av_playback_changed();
        break;
    case ESP_AVRC_RN_PLAY_POS_CHANGED:
        ESP_LOGI(BT_AV_TAG, "Play position changed: %lu-ms", event_parameter->play_pos);
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_REMOTE_FEATURES_EVT: {
        ESP_LOGI(BT_RC_CT_TAG, "AVRC remote features %x, TG features %x", (int)rc->rmt_feats.feat_mask, (int)rc->rmt_feats.tg_feat_flag);
        break;
    }
    case ESP_AVRC_CT_GET_RN_CAPABILITIES_RSP_EVT: {
...
static void bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_TG_REGISTER_NOTIFICATION_EVT: {
        ESP_LOGI(BT_RC_TG_TAG, "AVRC register event notification: %d, param: 0x%x", rc->reg_ntf.event_id, (int)rc->reg_ntf.event_parameter);
    ...
    case ESP_AVRC_TG_REMOTE_FEATURES_EVT: {
        ESP_LOGI(BT_RC_TG_TAG, "AVRC remote features %x, CT features %x", (int)rc->rmt_feats.feat_mask, (int)rc->rmt_feats.ct_feat_flag);
        break;
    }
    default:
